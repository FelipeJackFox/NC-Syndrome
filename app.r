...
run_stochastic <- function(population, init_I, init_E, init_R, gamma_base, sigma_base, nu, vaccination, R0,
  ...
  sim_one <- function(id_sim) {
    ...
    data.frame(time = t_vec, S = S, E = E, I = I, R = R, Beta = beta_tr, Sim = paste0("Sim ", id_sim))
  }
...
badge <- function(text, color = "#1e88e5") {
  tags$span(text, class = "badge-pill px-2 py-1", style = paste0("background-color:", color, "; color:white;"))
}
        ...
        fluidRow(
          column(7,
                 tags$div(style = "border:1px solid #174b7a; border-radius:12px; overflow:hidden;",
                          tags$iframe(src = "Situación Problema.pdf", width = "100%", height = "780px", style = "border:none; background:#0f2033;")
                 )
          ),
          column(5,
                 div(class = "info-card",
        ...
        fluidRow(
          ...
          column(3,
                 wellPanel(
                   ...
                   sliderInput("step", "Paso de tiempo (días)", min = 0.1, max = 5, value = 1, step = 0.1),
                   checkboxGroupInput("compartments", "Compartimentos a mostrar", choices = c("S", "E", "I", "R"), selected = c("S", "E", "I", "R")),
                   checkboxGroupInput("modelo_sel", "Modelo a visualizar", choices = c("Determinista", "Estocástico"), selected = c("Determinista", "Estocástico")),
                   checkboxInput("auto_refresh", "Refresco automático (estocástico)", TRUE),
                   actionButton("simulate", label = "Simular ahora", icon = icon("play"), class = "btn-primary btn-block")
                 )
          ),
          column(4,
                 wellPanel(
                   ...
                   sliderInput("det_gamma", "Gamma (recuperación)", min = 0.05, max = 0.3, value = 0.143, step = 0.001),
                   sliderInput("det_sigma", "Sigma (latencia)", min = 0.05, max = 0.6, value = 0.25, step = 0.01),
                   sliderInput("det_nu", "Natalidad/Mortalidad (nu)", min = 0, max = 0.001, value = 0.0000245, step = 0.00001),
                   sliderInput("det_v", "Vacunación", min = 0, max = 0.05, value = 0, step = 0.001),
                   sliderInput("det_I0", "Infectados iniciales", min = 1, max = 1000, value = 1),
                   sliderInput("det_E0", "Expuestos iniciales", min = 0, max = 2000, value = 0),
          ...
          column(5,
                 wellPanel(
                   ...
                   sliderInput("stoc_nu", "Natalidad/Mortalidad (nu)", min = 0, max = 0.001, value = 0.0000245, step = 0.00001),
                   sliderInput("stoc_v", "Vacunación", min = 0, max = 0.05, value = 0, step = 0.001),
                   sliderInput("stoc_I0", "Infectados iniciales", min = 1, max = 2000, value = 5),
                   sliderInput("stoc_E0", "Expuestos iniciales", min = 0, max = 2000, value = 0),
                   sliderInput("stoc_R0_init", "Recuperados iniciales", min = 0, max = 5000, value = 0),
                   sliderInput("stoc_season_strength", "Fuerza estacional", min = 0, max = 1, value = 0.4, step = 0.05),
                   sliderInput("stoc_season_peak", "Pico estacional (día)", min = 0, max = 365, value = 75, step = 1),
                   sliderInput("stoc_period", "Periodo estacional (días)", min = 30, max = 365, value = 365, step = 5),
                   ...
                   sliderInput("stoc_sims", "Número de simulaciones", min = 1, max = 30, value = 5, step = 1)
                 )
          )
        ),
        hr(),
...
server <- function(input, output, session) {
  ...
  observe({
    if (isTRUE(input$auto_refresh)) autoTimer();
  ...
  output$beta_plot <- renderPlotly({
    ...
    beta_df <- st |>
      ...
      mutate(Tendencia = input$stoc_R0 * input$stoc_gamma * (1 + input$stoc_season_strength * cos(2 * pi * (time - input$stoc_season_peak) / input$stoc_period)))

    p <- ggplot(beta_df, aes(x = time)) +
      geom_line(aes(y = Beta_prom), color = "#ce93d8", size = 0.8) +
      geom_line(aes(y = Tendencia), linetype = "dashed", color = "#90caf9", size = 0.8) +
      labs(title = "Tasa de contagio (β)", subtitle = "Promedio estocástico vs. tendencia estacional", y = "β", x = "Tiempo") +
      theme_minimal(base_size = 13) +
      theme(plot.background = element_rect(fill = "#0f2033", colour = "#0f2033"),
            panel.background = element_rect(fill = "#0f2033", colour = NA),
            text = element_text(colour = "#e5ecf4"))
  ...
  output$error_plot <- renderPlot({
    ...
    ggplot(det, aes(x = time, y = error_balance)) +
      ...
      labs(title = "Error de balance poblacional (Determinista)", x = "Tiempo", y = "S+E+I+R - N") +
  ...
  observeEvent(combined_series(), {
    ...
    render_mini_box <- function(title, value, accent = "#1e88e5") {
      div(class = "mini-box",
          ...
          div(style = paste0("height:6px; background:linear-gradient(90deg,", accent, ", #0f2033); border-radius:10px; margin-top:8px;"))
      )
    }

    output$peak_inf <- renderUI({
      render_mini_box("Pico de infectados", scales::comma(round(inf_curve$Valor)), "#1e88e5")
    })
    output$peak_day <- renderUI({
      render_mini_box("Momento del pico", paste0(round(inf_curve$time), " días"), "#26c6da")
    })
    output$final_R <- renderUI({
      render_mini_box("Recuperados finales", scales::comma(round(rec_final$valor)), "#43a047")
    })
  })
}
